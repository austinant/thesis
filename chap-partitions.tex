...
Several of their results (which will be addressed further in Chapter \ref{ch:applications}) specify some elements of $\PN$ with very well-controlled sets of factorizations.
To contrast with this, they also proved \cite[Proposition 4.8]{fan-tringali18}, which says that $\mathsf{L}_\PN(\llb 0,n \rrb) = \llb 2,n \rrb$ for every $n\ge 2$.
In essence, the intervals $\llb 0,n \rrb\in \PN$ seem to have the most wild factorization behavior.
However, as points are removed from $\llb 0,n \rrb$, one expects a transition to relative tameness as the set of factorizations becomes smaller.
...
\section{Algorithmic Approaches and Partition Type}
...
\begin{eg}
	Let $H = \NN$ and take $X := \llb 0,n \rrb$ for some odd $n \ge 11$.
	We can show that $A := \{0,1,3\}$ and $B := \{0,1,3,\dots,n-2,n-3\}$ are both atoms, and that $X = A + B$ (so $A*B$ is a factorization of $X$).
	However, $X\:A = \llb 0,n \rrb \cap \llb -1,n-1 \rrb \cap \llb -3,n-3 \rrb = \llb 0,n-3 \rrb \neq B$ and $X\: B \neq A$.
	Thus the above algorithm will find factorizations of the form $A*\bb$ for $\bb$ a factorization of $\llb 0,n-3 \rrb$ (and similarly $B*\aa$ for $\aa$ a factorization of $\llb 0,3 \rrb$), but it will fail to find $A*B$.
\end{eg} 
...
Note that 
\begin{itemize}
	\item $\PN = \bigsqcup_{n=0}^\infty \P_n$.
	\item For any $m,n\ge 0$, $\P_m + \P_n \subseteq \P_{m+n}$.
\end{itemize}
...
This will help us in our endeavor to develop a more effecient algorithm for factoring in $\PN$, and also in our larger goal to quanitfy the ``wildness" of factorizations of $\llb 0,n\rrb$.
...
It also supposes that one has found the sets $\A_m$ for $m < \max(X)$.
...
For every $(A_1,\dots,A_k)\in \A_{m_1}\times\cdots\times\A_{m_k}$, if $A_1 + \cdots + A_k = X$, then add $A_1 * \cdots * A_k$ to $\mathbf{fac\_of\_type}(X,P)$.
