...
\begin{defn} \label{def:partition type}
	...
	\[ \Z^P(X) := \{ A_1*\cdots * A_k \in \Z(X) : A_i \in \mathcal{A}_{m_i} \textrm{ for } i\in [ 1,k ] \}. \]
...
\begin{lemma} \label{lem:two-part types}
	If $q\ge r \ge 3$ then $[ 0, q+r ] \in \A_q + \A_r$; that is, there are atoms $A_q\in \A_q$ and $A'_r\in \A_r$ such that $A_q + A'_r = [ 0,q+r ]$.
\end{lemma}
...
\begin{lemma} \label{lemma:many 2s types}
	For $q \ge 6$ and $t\ge 2$, there is an atom $A\in \A_q$ with $A + t\{0,2\} = [ 0, q+2t ]$.
\end{lemma}
...
\begin{proof}
	...
	Similarly, if $k=2$ we have by Lemma \ref{lem:two-part types} that there are atoms $A\in \A_{m_1}$ and $A'\in \A_{m_2}$ with $A+A' = [ 0,m_1+m_2 ]$, so $A*A'* \{0,1\}\in \Z^P([ 0,n ])$.
